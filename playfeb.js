...
handlers.mintNFT = function (args, context) {
    try {
        ...
        // Get backend service URL from args, Title Data, or config
        var backendServiceUrl = args.backendServiceUrl;
        if (!backendServiceUrl) {
            try {
                var titleData = server.GetTitleData({ Keys: ["BACKEND_SERVICE_URL"] });
                if (titleData.Data && titleData.Data["BACKEND_SERVICE_URL"]) {
                    backendServiceUrl = titleData.Data["BACKEND_SERVICE_URL"];
                }
            } catch (e) {
                log.debug("Could not fetch BACKEND_SERVICE_URL from Title Data");
            }
        }
        
        // If backend service URL is available, call it to execute the transaction
        if (backendServiceUrl) {
            try {
                var backendRequest = {
                    action: "mint",
                ...
                var responseData = JSON.parse(backendResponse.Data || "{}");
                
                // Update player data with success status
                if (responseData.success && responseData.transactionDigest) {
                    server.UpdateUserInternalData({
                        PlayFabId: currentPlayerId,
                        Data: {
                            lastMintRequest: JSON.stringify({
                                ...
                                nftName: args.name,
                                status: "completed",
                                transactionDigest: responseData.transactionDigest,
                                nftObjectId: responseData.nftObjectId,
                                kioskId: responseData.kioskId,
                                kioskOwnerCapId: responseData.kioskOwnerCapId
                            })
                        }
                    });
                }
                ...
                return {
                    ...
                    message: responseData.success ? "✅ NFT minted successfully on blockchain!" : "Mint request sent to backend service"
                };
            } catch (httpError) {
                ...
                return {
                    ...
                    message: "❌ Failed to execute mint. Backend service error: " + httpError.message,
                    instructions: "Check your backend service URL and ensure it's running. Transaction data is prepared but not executed."
                };
            }
        }
        
        // No backend service configured - return transaction data for manual execution
        log.warning("No backend service configured - transaction NOT executed", {
            playerId: currentPlayerId,
            network: networkInfo.network,
            message: "Set backendServiceUrl in args or BACKEND_SERVICE_URL in Title Data"
        });
        ...
        return {
            ...
            network: networkInfo.network,
            transactionData: transactionData,
            message: "❌ TRANSACTION NOT EXECUTED - No backend service configured",
            error: "Backend service URL not found",
            instructions: [
                "1. Start backend service: cd playfeb && npm start",
                "2. Get public URL (use ngrok for testing: ngrok http 3000)",
                "3. In PlayFab: Content > Title Data > Add 'BACKEND_SERVICE_URL' = 'http://your-url:3000/mint'",
                "OR pass backendServiceUrl in function arguments"
            ],
            transactionDataOnly: true
        };
    ...
    try {
        ...
        if (backendServiceUrl) {
            try {
                ...
                var backendResponse = http.request(
                    backendServiceUrl.replace('/mint', '/transfer'), // Use transfer endpoint
                    "post",
                    JSON.stringify(backendRequest),
                    "application/json",
                    {
                ...
                return {
                    ...
                    transactionDigest: responseData.transactionDigest,
                    explorerUrl: responseData.explorerUrl,
                    message: responseData.success ? "✅ NFT transferred successfully on blockchain!" : "Transfer request sent to backend service"
                };
            } catch (httpError) {
                log.error("Error calling backend service", {
                    error: httpError.message,
                ...
                return {
                    ...
                    transactionData: transactionData,
                    backendError: httpError.message,
                    message: "❌ Failed to execute transfer. Backend service error: " + httpError.message
                };
            }
        }
        ...
        return {
            ...
            network: networkInfo.network,
            transactionData: transactionData,
            message: "⚠️ Transaction data prepared but NOT executed. Set backendServiceUrl or BACKEND_SERVICE_URL in Title Data to auto-execute."
        };
